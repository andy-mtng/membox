function InformationBox({ message, type }) {
    ...
    const getClassIcon = () => {
      switch (type) {
        case 'success':
          return (
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 text-green-800">
    ...
    return (
        <div className={`font-medium flex gap-2 absolute right-4 top-4 z-50 p-2 rounded-md border ${styles}`}>
            {icon}
            <h1>{message}</h1>
        </div>
    )
