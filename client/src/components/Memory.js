        ...
        <div className={`box-border relative shadow-md w-80 h-60 ${isCoreMemory ? "border-2 border-yellow-400" : "border border-gray-200"}`}>
            ...
            <div className="relative h-2/3 overflow-hidden">
                ...
                <div className='z-40 absolute top-3 left-3'>
                    ...
                    <h2 className="font-normal text-sm text-gray-300">{DOMPurify.sanitize(formattedDate)}</h2>
                </div>
                <div className="absolute inset-0 bg-black opacity-60"></div>
            </div>
            <p className="px-3 py-3 font-normal text-sm text-black mt-auto break-words">{DOMPurify.sanitize(description)}</p>
        </div>
    )
}
